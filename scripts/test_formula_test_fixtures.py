"""
Tests for scripts/formula_parser.py.

`formula_parser` is imported directly by scripts/validate_formulae.py (the
module Homebrew CI's *Validate Formulae* job runs) and by ~25 invariant
test modules — its `load_formulae()` loader is on the hot path for every
...
The historical ``ReExportShimTests`` class was retired in kubestellar/
homebrew-tap#570 after the ``formula_test_fixtures.py`` re-export block
was removed; every invariant test module now imports parser symbols from
``formula_parser`` directly, so there is no re-export shim left to pin.
"""
    ...
    def test_returns_stem_to_text_map_for_populated_dir(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(formula_parser, "FORMULA_DIR", tmp_path):
                result = formula_parser.load_formulae()
...
class ListFormulaPathsTests(unittest.TestCase):
    def test_returns_sorted_paths_for_populated_dir(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (tmp_path / "alpha.rb").write_text("class Alpha < Formula\nend\n", encoding="utf-8")
            with mock.patch.object(formula_parser, "FORMULA_DIR", tmp_path):
                result = formula_parser.list_formula_paths()
    ...
    def test_empty_formula_dir_raises_assertion_error_with_path(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(formula_parser, "FORMULA_DIR", tmp_path):
                with self.assertRaises(AssertionError) as cm:
                    formula_parser.list_formula_paths()
            self.assertIn(str(tmp_path), str(cm.exception))
